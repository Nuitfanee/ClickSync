...
- **高级参数**：不同设备会自动显示/隐藏不兼容项（例如 Rapoo/ATK 家族的差异项）。
...
├── protocol_api_rapoo.js       # Rapoo 家族协议实现（分层：Driver/Codec/Planner/SPEC/API）
├── protocol_api_atk.js         # ATK 协议实现（如仓库中存在）
├── protocol_api_chaos.js       # Chaos 协议实现（如仓库中存在）
├── refactor.js                 # 全局配置 + 设备适配器/写入策略 + UI Variant 应用
├── mouse-main.js               # 测试工具主逻辑（如仓库中存在）
...
### 3）协议实现的分层（以 `protocol_api_rapoo.js` 为例）
